...
import com.rxxb.imagepicker.crop.util.MatrixUtils;
import com.rxxb.imagepicker.crop.util.TensionInterpolator;
...
class CropIwaImageView extends AppCompatImageView implements OnNewBoundsListener, ConfigChangeListener {
    ...
    private void animateToAllowedBounds() {
        ...
        updateImageBounds();
        invalidate();
        /*final float initialAngle = new MatrixUtils().getMatrixAngle(imageMatrix);
        System.out.println("旋转角度:"+initialAngle);
        ...
        animator.animate(imageMatrix, endMatrix, new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                Matrix newMatrix = (Matrix) animation.getAnimatedValue();
                imageMatrix.set(newMatrix);
                if (initialAngle > 0) {
                    imageMatrix.postRotate(initialAngle, allowedBounds.centerX(), allowedBounds.centerY());
                }
                setImageMatrix(imageMatrix);
                updateImageBounds();
                invalidate();
    ...
    public void rotateImage(float deltaAngle, float px, float py) {
        imageMatrix.postRotate(deltaAngle, px, py);
        setImageMatrix(imageMatrix);
        updateImageBounds();
    }
