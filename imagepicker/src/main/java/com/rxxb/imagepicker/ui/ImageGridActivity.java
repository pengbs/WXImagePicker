...
public class ImageGridActivity extends ImageBaseActivity implements ImageDataSource.OnImagesLoadedListener,
        ImageRecyclerAdapter.OnImageItemClickListener, ImagePicker.OnImageSelectedListener, View.OnClickListener , CompoundButton.OnCheckedChangeListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mFooterBar.setBackgroundColor(Color.parseColor(imagePicker.getViewColor().getToolbarBgColor()));
        mBtnPre.setTextColor(Color.parseColor(imagePicker.getViewColor().getToolbarTitleColorDisabled()));
        mCbOrigin.setTextColor(Color.parseColor(imagePicker.getViewColor().getToolbarTitleColorNormal()));
        mtvDir.setTextColor(Color.parseColor(imagePicker.getViewColor().getToolbarTitleColorNormal()));
...
//            intent.putExtra(ImagePicker.EXTRA_IMAGE_ITEMS, imagePicker.getCurrentImageFolderItems());
            ...
            if (imagePicker.isCrop()) {
                startActivityForResult(CropActivity.callingIntent(this, Uri.fromFile(new File(imageItem.path))), ImagePicker.REQUEST_CODE_CROP);
            } else {
                Intent intent = new Intent();
                intent.putExtra(ImagePicker.EXTRA_RESULT_ITEMS, imagePicker.getSelectedImages());
    ...
    public void onImageSelected(int position, ImageItem item, boolean isAdd) {
        if (imagePicker.getSelectImageCount() > 0) {
            ...
            mBtnPre.setTextColor(Color.parseColor(imagePicker.getViewColor().getToolbarTitleColorNormal()));
            mBtnOk.setTextColor(Color.parseColor(imagePicker.getViewColor().getToolbarTitleColorNormal()));
        } else {
            ...
            mBtnPre.setTextColor(Color.parseColor(imagePicker.getViewColor().getToolbarTitleColorDisabled()));
            mBtnOk.setTextColor(Color.parseColor(imagePicker.getViewColor().getToolbarTitleColorDisabled()));
        }
